...
class FlixHQAPI:
    ...
    def __init__(self):
        ...
        if chrome_binary:
            ...
            logger.info(f"✓ Chrome at: {chrome_binary}")
    ...
    def get_details_with_servers(self, movie_url):
        try:
            ...
            try:
                ...
                for idx, button in enumerate(server_buttons):
                    try:
                        ...
                        if server_name:
                            logger.info(f"🎬 Found {server_name}! Clicking...")
                            ...
                            try:
                                ...
                                if iframe:
                                    ...
                                    logger.warning(f"⚠️ No iframe for {server_name}")
                            
                            except Exception as click_err:
                                logger.warning(f"⚠️ Click failed: {click_err}")
                    
                    except Exception as btn_err:
                        continue
            
            except Exception as selenium_err:
                logger.error(f"❌ Extraction failed: {selenium_err}")
            ...
            details['servers'] = unique_sources
            details['server_count'] = len(unique_sources)
            
            logger.info(f"🎉 Total: {len(unique_sources)} servers")
...
def home():
    return jsonify({
        'name': 'FlixHQ API',
        'version': '5.0.0',
        'message': '🎬 UpCloud, VidCloud & MegaCloud support!',
        'servers': ['UpCloud', 'VidCloud (AkCloud)', 'MegaCloud'],
        'endpoints': {
            'health': '/api/health',
...
if __name__ == '__main__':
    print("=" * 60)
    print("🎬 FlixHQ API v5.0 - UpCloud, VidCloud & MegaCloud")
    print("=" * 60)
    print("\n📡 Endpoints:")
    ...
    print("\n🎯 Supports: UpCloud, VidCloud (AkCloud), MegaCloud")
